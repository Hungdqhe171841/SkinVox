...
router.get('/products', async (req, res) => {
  try {
    ...
    const buildQuery = (baseQuery = {}) => {
      let query = { isActive: true, ...baseQuery };
...
router.get('/stats', async (req, res) => {
  try {
    const [
      ...
      Lipstick.countDocuments({ isActive: true }),
      Eyeshadow.countDocuments({ isActive: true }),
      Blush.countDocuments({ isActive: true }),
      Eyebrows.countDocuments({ isActive: true }),
      Eyeliner.countDocuments({ isActive: true })
    ]);
